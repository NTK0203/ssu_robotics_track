...
### 주요 기능
- **Substitutions (치환)**: 실행 시점(Launch time)에 평가되는 기능입니다. `FindPackageShare`와 같은 기능을 사용하여 특정 패키지의 설치 경로를 하드코딩하지 않고 동적으로 찾아낼 수 있습니다. 이를 통해 환경이 변해도 코드 수정 없이 실행이 가능합니다.
