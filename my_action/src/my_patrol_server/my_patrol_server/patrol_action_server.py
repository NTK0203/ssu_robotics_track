...
class PatrolActionServer(Node):
    ...
    def execute_callback(self, goal_handle):
        # 이 예시에서는 타이머에서 로직을 처리하므로 즉시 빈 결과 반환
    ...
    def timer_cb(self):
        ...
        ang_err = target_angle - self.pose.theta
        ang_err = math.sin(target_angle - self.pose.theta)
        # math.cos(target_angle - self.pose.theta) # 이미지 상에는 존재하나 할당 없음
