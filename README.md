...
이 리포지토리는 **SSU 로보틱스 트랙** 과정을 학습하며 ROS2를 활용한 로봇 모델링, 시뮬레이션, 시각 제어 및 산업용 협동 로봇 제어를 학습한 기록을 담고 있습니다. 기초적인 통신 구조부터 MoveIt2를 활용한 경로 계획, 그리고 인공지능 기반의 STT 및 Vision 연동 실습과 NVIDIA Isaac Sim을 이용한 시뮬레이션 내용을 포함하고 있습니다.

# 🛠️ Tech Stacks

- **Languages**: Python, Xacro/XML (URDF)
- **Platform**: ROS2 Humble (Ubuntu 22.04), Docker
- **Simulation**: Gazebo, RViz2, MoveIt2, NVIDIA Isaac Sim
- **Hardware & Library**: Doosan Robotics API (M0609), Intel RealSense (Depth Camera), OpenCV, Isaac Sim Core API
...
가상 환경에서 물리 엔진을 연동하여 로봇의 정밀 제어를 구현했습니다.
...
### 4. NVIDIA Isaac Sim Python Scripting
고성능 시뮬레이터를 활용한 로봇 제어 스크립팅 기법을 학습했습니다.
- **Core API 활용**: Isaac Sim의 World와 Scene 인터페이스를 통해 가상 환경을 구축하고 Jetbot 로봇 모델을 동적으로 스폰 및 관리.
- **Physics-based Control**: 물리 콜백(Physics Callback) 함수와 비차단식 시간 제어 로직을 구현하여 정교한 자율 주행 시나리오 설계.
...
┣ 📂 user_examples      # NVIDIA Isaac Sim Python 스크립팅 및 Jetbot 자율 주행
┣ 📂 ros_ws             # 해당 디렉토리는 레포 업로드에 생략
